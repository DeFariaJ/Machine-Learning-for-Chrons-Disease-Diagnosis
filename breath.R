...
breathD <- readMat("/Users/goncaloleiria/Desktop/gcms_data/breath/BWG_BR_CDvCTRL.mat")

#getting essential elements for machine learning
X <-breathD$XTIC #GC-MS data for each sample
y <-breathD$CLASS #Disease state (1 = Control, 2 = Crohn's Disease (CD))

# copy sample names from SAM to row names of X
rownames(X) <-as.character(unlist(breathD$SAM))
...
#MACHINE LEARNING#
test1_breath <- cfBuild(subset_PCA, inputClass = y, bootNum = 50, ensNum = 50, parallel = TRUE, cpus = 6)

#PERMUTATION#
test1_Permut_Breath <- cfPermute(subset_PCA, inputClass = y, bootNum = 50, ensNum = 50, permNum = 50, parallel = TRUE, cpus = 6)

#creating dataframe with Acc test and Acc permutation
Ensemble <- test1_breath[["testAcc"]]
Acc_table <- as.data.frame(Ensemble)
Acc_table$Permutation <- test1_Permut_Breath[["avgAcc"]]
par(mfrow=c(1,2))
#####
...
#statistical test
Stattest2 <-t.test(Acc_table$Ensemble,Acc_table$Permutation, var.equal = TRUE)
Stattest2
