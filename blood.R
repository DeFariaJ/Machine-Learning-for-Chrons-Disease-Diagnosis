...
bloodD <- readMat("/Users/goncaloleiria/Desktop/gcms_data/blood/BWG_BL_CDvCTRL.mat")

#getting essential elements for machine learning
X <-bloodD$XTIC #GC-MS data for each sample
y <-bloodD$CLASS #Disease state (1 = Control, 2 = Crohn's Disease (CD))

# copy sample names from SAM to row names of X
rownames(X) <-as.character(unlist(bloodD$SAM))
...
subset_PCA <- mat[c(1:2)] #2 first PC account for 94% of the variation

###########################
#MACHINE LEARNING#     SVMs
############################
test1_blood <- cfBuild(subset_PCA, inputClass = y, bootNum = 50, ensNum = 50, parallel = TRUE, cpus = 6)

############################
#PERMUTATION#
############################
test1_Permut_Blood <- cfPermute(subset_PCA, inputClass = y, bootNum = 50, ensNum = 50, permNum = 50, parallel = TRUE, cpus = 6)

#creating dataframe with Acc test and Acc permutation
Ensemble <- test1_blood[["testAcc"]]
Acc_table <- as.data.frame(Ensemble)
Acc_table$Permutation <- test1_Permut_Blood[["avgAcc"]]
par(mfrow=c(1,2))
#####
...
#statistical test
Stattest3 <-t.test(Acc_table$Ensemble,Acc_table$Permutation, var.equal = TRUE)
Stattest3
